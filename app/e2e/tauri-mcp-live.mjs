...
const resultPath = resolve(
  process.env.YUANBAO_TAURI_E2E_RESULT_PATH ||
    `${tempRoot}/yuanbao-tauri-mcp-live-result.json`,
);
const databasePath = resolve(
  process.env.YUANBAO_TAURI_E2E_DB_PATH ||
    `${tempRoot}/yuanbao-tauri-mcp-live.sqlite3`,
);
...
try {
  ...
  if (!existsSync(resultPath)) {
    console.error(`Tauri MCP live E2E did not produce a result within ${timeoutMs}ms.`);
