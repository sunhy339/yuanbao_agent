...
- OpenAI Responses API 的工具参数流也已进入统一事件流：`function_call_arguments.delta` 会转成 `tool_call_delta`，再生成 clean 可消费的工具占位和 `toolInput` 增量；流式工具输入的 `content_start/content_delta` 已提前带 `parentToolUseId/toolCategory/toolPhaseId/toolPhaseLabel/toolSemanticParentId/toolSemanticParentLabel`，和 OpenAI Chat SSE 的体感一致且早期占位就能按后端语义阶段归组。
- OpenAI Responses API 的 reasoning summary 与非流式 provider 思考摘要都已进入统一事件流：`reasoning_summary_text.delta` 会转成 `thinking` 事件，非流式 provider 明确返回的 `thought_summary/thoughtSummary` 会桥成 `source=thought_summary` 的 thinking，clean 前端按同一 task 累积为轻量思考行，不混入最终回答正文。
...
| 上下文按钮 | 上下文占用按钮，弹出详细 breakdown | 显示上下文百分比/标签，已补 token 预算、工具数、当前步骤、压缩节段、上下文组成条、纳入章节 chips 和系统提示层，并可复制上下文摘要 | contextPreview / budgetStats.includedSections / promptLayers | `部分接入`：分类 breakdown 已有轻量版，仍缺更完整持久快照 |
...
| 上下文按钮 | 百分比/状态，点击看详情 | 有百分比短显示、轻量详情浮层、分类 breakdown、纳入章节和复制上下文摘要 | contextPreview | `部分接入`：缺更完整持久快照 |
...
| 模型思考 | thinking 块，随流式更新 | 有 thinking/progress 入口，默认渲染为轻量细线行；OpenAI Responses reasoning summary delta 与非流式 provider `thought_summary/thoughtSummary` 会进入 `thinking` 并按 task 累积 | metadata/status/thinking event | `部分接入`：Responses 流式摘要和非流式显式思考摘要已接，其他 provider 的 token 级 thinking delta 和更细阶段仍待补 |
...
| `message_complete` | 本轮消息结束 | task lifecycle `message.completed` -> chat-compat `message_complete` | `已接入`：后端完成任务时会发布，前端收到后清理 thinking/streaming 占位 |
| `thinking` | 思考内容 | status/thinking metadata + OpenAI Responses reasoning summary delta + non-stream `thought_summary` | `部分接入`：Responses reasoning summary 和非流式显式 thought summary 已接；其他 provider 的 token 级 thinking 与更完整思考阶段仍待补 |
...
1. `后端事件流`：让 assistant 正文/thinking/tool/status 按时间进入 transcript，而不是最后汇成一大段。前端已加 adapter，可先吃部分 haha-cc 风格事件；实时 EventBus 已补单调时间戳和 `seq` 透出，messages 默认有 `createdSeq`，下一步继续补更细的 provider/工具结果分段。
