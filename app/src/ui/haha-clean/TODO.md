...
- Yuanbao flat 输出链继续补齐：实时 `AgentEventEnvelope` 只新写并只读取 `yuanbao` 扁平 ServerMessage；legacy `hahaCc` 字段只作为历史迁移术语/负向断言保留，不再作为新事件、新 stdout frame、新桌面通道或 replay fallback。Tauri 只广播 `yuanbao://message`，stdio 只写出 `kind=yuanbao_message`；`trace.list`、`events.after` 和 replay timeline 读取同一份 flat message，补拉入口统一为后端 `events.yuanbaoAfter` / 桌面 `yuanbao_events_after` / `RuntimeClient.yuanbaoEventsAfter()`。
- shared `YuanbaoServerMessage` 已按 `docs/cc-haha-main` 的原始 `ServerMessage` 收紧：flat message 只声明 haha-cc 同形字段，本地扩展字段继续留在 `AgentEventEnvelope.payload`，避免外部 adapter 把本地 payload 当成协议能力。
- 连接/心跳入口也补到桌面桥：后端 `runtime.ping` 会返回 `connected/pong` 扁平 ServerMessage，Tauri 暴露 `runtime_ping`，前端 `RuntimeClient.runtimePing()` 可直接拿到同形消息；`connectYuanbaoMessages()` 会在订阅 flat 通道后先派发 `connected+pong`，再按间隔发送 keepalive `pong`。
