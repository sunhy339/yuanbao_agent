...
- Yuanbao flat 低卡片消息流：过程说明、工具行、文件改动、上下文压缩/Goal/Memory/API retry 等事件均有前端渲染入口。
...
- Yuanbao flat 输出链继续补齐：实时 `AgentEventEnvelope` 会携带 `yuanbao` 扁平 ServerMessage，并保留 legacy `hahaCc` 兼容字段；Tauri 额外广播 `yuanbao://message`，同时保留 `haha-cc://message` 兼容通道，stdio 也会写出 `kind=yuanbao_message` 和 `kind=haha_cc_message`；`trace.list`、`events.after` 和 replay timeline 现在会返回同一份 flat message，并新增后端 `events.hahaCcAfter` / 桌面 `haha_cc_events_after` / `RuntimeClient.hahaCcEventsAfter()` 纯 flat 补拉入口，刷新、补拉和回放不再退回另一套输出形状，外部 adapter 也不必自行剥 `AgentEventEnvelope`。
...
- Yuanbao flat 输出继续收紧到真实后端流程：`task.updated` 实时事件会携带 `task_update`，协作 RPC 的 `collab.task.created/claimed/message.sent/completed` 会携带 `team_created/team_update` 并同步进入 trace；`session.update` 会发布带 `changedFields` 的 `session.updated`，只有标题真的变化时才派生 `session_title_updated`，摘要/记忆刷新不再误报标题更新。
