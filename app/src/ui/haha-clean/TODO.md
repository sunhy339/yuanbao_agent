...
- 真实 token 级 thinking/assistant delta 分段，而不是只在最终消息里得到大段总结。
- 工具调用 parent/child 树、每个工具的结构化 input/output summary。
...
- Computer Use 权限请求、ask_user_question、goal_event、memory_event、compact_summary、api_retry 这些事件目前前端已预留类型，但后端还没有等价事件流。
