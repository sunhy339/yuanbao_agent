...
export const CLEAN_TRANSCRIPT_CAPABILITIES: CleanTranscriptCapability[] = [
  ...
  { kind: "tool_group", label: "工具分组/树", current: "partial", source: "worklog grouping; parentToolUseId needs backend persistence" },
  { kind: "permission_request", label: "权限/审批请求", current: "mapped", source: "permission_request/approvals" },
  { kind: "computer_use_permission", label: "Computer Use 权限", current: "partial", source: "computer_use_permission payload can render; approval dialog/submit still needs backend" },
  { kind: "ask_user_question", label: "向用户提问", current: "partial", source: "ask_user_question payload can render; answer submit still needs backend" },
  { kind: "background_task", label: "后台/子任务", current: "partial", source: "backgroundJobs/collaboration; child task stream needs backend" },
  { kind: "task_summary", label: "任务摘要", current: "partial", source: "activeTask plan/summary; final timing needs backend" },
  { kind: "plan_update", label: "计划更新", current: "partial", source: "activeTask.planSteps/task.updated" },
