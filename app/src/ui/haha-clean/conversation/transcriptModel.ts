...
export const CLEAN_TRANSCRIPT_CAPABILITIES: CleanTranscriptCapability[] = [
  ...
  { kind: "goal_event", label: "Goal 事件", current: "mapped", source: "missing goal lifecycle chat event" },
  { kind: "memory_event", label: "记忆/上下文事件", current: "mapped", source: "missing memory lifecycle chat event" },
  { kind: "compact_summary", label: "上下文压缩", current: "mapped", source: "missing compact_summary chat event" },
  { kind: "slash_command", label: "Slash command", current: "mapped", source: "local slash command dispatcher" },
  { kind: "api_retry", label: "API 重试", current: "mapped", source: "missing api_retry chat event" },
