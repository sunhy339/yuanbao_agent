...
Coverage status: the first GLM attempt did not exercise main workflow nodes because provider probe failed before `workspace.open` / `message.send`. The explicit `gpt-5.5` run did exercise the main long-flow path, but intentionally remains a failed smoke until the real model produces the required files, tests, and verification. The runtime outcome is now stricter: a write-oriented task with only summary evidence and no approval path must fail or request review instead of silently completing.
