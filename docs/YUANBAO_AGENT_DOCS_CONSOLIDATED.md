...
| Provider recovery | Baseline classification, provider-turn audit, and one-shot compacted-context retry are in place. Provider failures are now gated before advisory: no-partial timeout/network/rate-limit/server failures surface as failed runtime facts; context-too-large can do one objective compacted retry; stream failures only consult `DecisionAdvisor` or fall back to non-streaming when partial output already exists. Completed turns that used recovery retain `failureRecovery` audit. Remaining deeper work is preflight risk detection, bounded task splitting before provider failure, and true guarded provider switching when infrastructure exists. | GLM timeout without partial output fails clearly with recovery audit; oversized context gets a bounded compact retry; partial stream failure can produce an advisor-audited fallback instead of losing progress. |
...
| Provider failures are provider-specific and intermittent. | Classification plus partial-aware recovery gating are implemented: no-partial transport/provider failures do not ask the same failing provider for recovery advice. Remaining mitigation is preflight splitting and guarded provider switching. |
...
2. Provider recovery phase 2 remainder: preflight bounded task splitting and guarded provider fallback/switching before or beside failure, not after a provider has gone silent; continue auditing remaining fixed recovery rules while runtime enforces retry budgets, partial-output gates, and safety.
