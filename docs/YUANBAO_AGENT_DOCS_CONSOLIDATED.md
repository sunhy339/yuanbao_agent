...
1. Product acceptance phase 2 follow-up: blocking advisor-suggested commands and non-command tool suggestions can now turn into real approvals with advisor audit metadata and approval-resume execution. Remaining work is richer domain signal discovery and docs/readme quality checks while keeping the evidence shape advisor-led rather than taxonomy-coded.
