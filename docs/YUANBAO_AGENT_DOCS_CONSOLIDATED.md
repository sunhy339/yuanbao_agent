...
| Provider failure recovery advisor | Done in current follow-up | Provider failures are still classified by observable facts, but recoverable failures now call `DecisionAdvisor` with category/retryable/recoverable/error/request context. Accepted advisor proposals choose a bounded strategy such as retry, retry_with_backoff, compact_or_split_context, ask_user, abort, or stream fallback; runtime clamps non-retryable auth/refusal, unavailable fallback, and max retries. Advisor proposals and runtime-bounded recovery records are persisted in `proposal_records`; provider traces include `provider.failure.recovery_decision` and retry metadata. |
...
| Provider recovery | Failure classification remains objective; recoverable retry/backoff/compact/fallback decisions are now advisor-proposed where possible and runtime-bounded. | Remaining: task-splitting recovery and true guarded provider switching still need execution infrastructure; auth/refusal stay non-retryable and user-visible. |
...
| P1 | Provider failure recovery phase 2 | Advisor-guided bounded retry/backoff/compact/stream-fallback is implemented and tested, including malformed advisor fallback and auth non-retry. Remaining work: bounded task splitting and true guarded provider switching when appropriate. |
...
2. Provider recovery phase 2 remainder: bounded task splitting and guarded provider fallback/switching after advisor-guided recovery; continue auditing remaining fixed recovery rules while runtime enforces retry budgets and safety.
