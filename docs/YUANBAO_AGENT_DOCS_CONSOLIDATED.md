...
| Advisor-requested follow-up evidence | Evidence execution must not be hard-coded to frontend/API/browser probes, and must not auto-run LLM-suggested commands or tools. | Fourth remediation done: `suggestedCommand` still becomes a permission-gated `run_command` suggestion, and `suggestedTool` becomes a generic tool suggestion with registry/capability/PermissionEngine metadata. Advisor evidence requests may now carry multi-step `suggestedCommands` / `suggestedTools`; runtime normalizes them into independent evidence requests so each step gets its own permission/approval/execution state. Blocking command/tool suggestions create explicit advisor evidence approvals even when PermissionEngine says the capability is `allow`; deny still blocks execution. Approved commands/tools execute through the normal command/tool pipeline with advisor audit metadata, and evidence status is satisfied only when the matching execution succeeds. Nothing is auto-run before approval. |
