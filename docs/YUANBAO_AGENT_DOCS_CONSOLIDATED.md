...
3. Extend provider failure handling beyond the current classifier/fallback baseline without asking a failing provider to recover itself: transport retries stay below the first emitted event, runtime classifies hard failures, and advisor-led recovery only runs when partial output or other usable semantic evidence exists.
...
| Provider recovery | Baseline classification, provider-turn audit, one-shot compacted-context retry, and provider-call preflight are in place. Provider failures are now gated before advisory: no-partial timeout/network/rate-limit/server failures surface as failed runtime facts; context-too-large can do one objective compacted retry; stream failures only consult `DecisionAdvisor` or fall back to non-streaming when partial output already exists. Before a risky provider call, `provider_preflight` gives `DecisionAdvisor` the request facts plus provider profile health/ranking. Runtime may auto-apply safe context compaction, execute a validated bounded split through the existing planning/DAG/subagent path, or apply a guarded turn-scoped provider profile switch when the advisor supplies `fallbackProviderId` for an enabled, switch-eligible profile. User-intervention advice remains audited. Completed turns that used recovery retain `failureRecovery` audit. | GLM timeout without partial output fails clearly with recovery audit; oversized context gets a bounded compact retry; partial stream failure can produce an advisor-audited fallback instead of losing progress; large/risky preflight requests can be compacted, split into bounded subtasks, or switched to a configured fallback provider profile before the provider call with proposal/trace audit. Profile ranking and switch health are persisted in provider preflight trace/proposal records for cockpit visibility. |
...
| Product acceptance | Objective artifact checks, static route/content health, docs-quality observations, product-surface advisory, and advisor-requested evidence approvals are in place. Remaining work is richer domain signal discovery plus advisor-led real evidence execution adapters. | Acceptance can include unit tests, integration tests, browser review, service/API proof, persistence round-trip, document render, hardware proof, design review, benchmark, or MCP/tool evidence only when those are the right evidence for the product shape. |
...
| Model routing | Route phases to suitable models or providers: strong model for planning/review, cheaper stable model for bounded execution, vision/tool-capable model for evidence adapters when needed, and fallback provider when recoverable. | Provider choice is visible in run reports and failures can trigger conservative fallback behavior. |
...
2. Verify that MCP service results and MCP/tool failures enter the working context and compaction handoff instead of staying as isolated tool output.
3. Verify mixed workflows where skill guidance, MCP calls, local file edits, advisor-selected evidence adapters, and tests all participate in one user-facing task.
4. Add recovery behavior for missing skills, unavailable MCP servers, partial MCP responses, and tool permission denials. Failed tool/MCP calls now persist as structured `handoffSummary.failedTools`; remaining work is proactive fallback strategy selection.
...
| Acceptance panel | Show tests, type checks, advisor-selected tool/browser/document/MCP evidence, generated files, screenshots or rendered artifacts when relevant, risk notes, and final run report. | A single place to answer "is this actually done?" |
...
1. MCP + Skills acceptance coverage — **Partial / mostly done**:
   - Done: skill-routed MCP tool call, `inherit_mcp` policy, compaction survival, missing skill fallback context/event, failed MCP/tool handoff via `handoffSummary.failedTools`, structured failure kind/recovery hint for failed tools, and advisor-requested MCP evidence tools routed through ToolPolicyResolver plus explicit `advisor_tool` approval.
   - Remaining: turning unavailable server / partial response / permission-denied hints into advisor-selected recovery actions.
2. Main workflow state machine — **Partial / main baseline done**:
   - Done: `routing.mainWorkflow`, intent confidence, automation level, budgets, workspace/git snapshot, user takeover state, budget exhaustion convergence, advisor-audited wrap-up/change-target convergence, and cockpit-visible handoff controls for refresh/resume against paused resumable tasks.
   - Remaining: richer budget dimensions and deeper automation policy controls.
3. Provider error recovery — **Done for current baseline**:
   - Done: objective failure classification, no-partial hard-failure gating, adapter-level retry only before the first stream event, compacted-context retry for oversized context, partial-stream-only advisor/fallback recovery, auth/refusal non-retry, provider preflight facts/advisor audit, safe preflight context compaction, executable bounded preflight task splitting through existing planning/DAG/subagent execution, guarded turn-scoped provider profile switching, provider trace/proposal audit, and completed-turn recovery audit retention.
   - Remaining deeper work: provider-specific health decay/latency scoring and richer UI history. Baseline health/ranking is implemented: preflight facts include `providerProfileRanking`, unhealthy targets are rejected, and switch trace/proposal records include target health.
...
5. Generated-artifact / product acceptance — **Partial / advisor-led path active**:
   - Done: readable artifact/mojibake checks, static frontend route/content health, local asset checks, `node --check`, `/api/...` objective observations, docs-quality observations, changed-surface observations, product-surface advisor, `agent.evidence.requested`, `on_evidence_requested`, and permission-gated advisor command/tool approvals.
   - Remaining: advisor-led real evidence execution adapters when the task shape/advisor asks for them, plus continued tuning of objective surface discovery.
6. Run report and frontend cockpit — **Partial**:
   - Done: cockpit phase 1 status strip for task phase, completion gate, approvals, changed files, commands, verification ratio, failed signals, first acceptance issue, context budget, expandable drill-downs for acceptance/provider/MCP/context/workspace signals, and a first handoff actions panel with refresh/resume controls wired to the existing task lifecycle.
   - Remaining: richer automation controls, provider health/ranking history, and deeper run-report navigation.
...
| Provider failures are provider-specific and intermittent. | Classification plus partial-aware recovery gating are implemented: no-partial transport/provider failures do not ask the same failing provider for recovery advice. Preflight now asks advisor before risky calls and can auto-apply safe compaction, execute a validated bounded split through planning/DAG, or apply a guarded turn-scoped switch to an enabled switch-eligible profile ranked by health. Remaining mitigation is richer provider-specific health history and UI drill-down. |
...
| Provider failure recovery advisor | Done in current follow-up | Provider failures are still classified by observable facts, but advisor recovery is now gated by usable evidence. No-partial timeout/network/rate-limit/server failures do not call the same provider-backed advisor or auto-retry at runtime; oversized context can receive a bounded runtime compact retry; stream failures only expose advisor/fallback recovery when partial output already exists. Adapter-level stream retry is limited to failures before the first emitted event. Provider preflight now records request facts before the provider call, consults `DecisionAdvisor` when risk signals warrant it, persists `provider_preflight` proposals for material decisions, emits `agent.decision.provider_preflight` / `provider.preflight.decision`, and can auto-apply safe context compaction, validated bounded splitting, or a guarded turn-scoped switch to a configured available provider profile. Advisor proposals and runtime recovery records are persisted in `proposal_records`; provider traces include `provider.failure.recovery_decision`, `advisorGate`, retry metadata, provider switch metadata, and completed turns retain `failureRecovery` when recovery happened before success. |
...
| Advisor-requested follow-up evidence | Evidence execution must not be hard-coded to frontend/API/browser probes, and must not auto-run LLM-suggested commands or tools. | Fifth remediation done: `suggestedCommand` still becomes a permission-gated `run_command` suggestion, and `suggestedTool` becomes a generic tool suggestion with registry/capability/PermissionEngine metadata. Advisor evidence requests may now carry multi-step `suggestedCommands` / `suggestedTools`; runtime normalizes them into independent evidence requests so each step gets its own permission/approval/execution state. Dynamic MCP suggested tools are no longer rejected merely because they lack a static capability mapping: registered `mcp__...` tools pass through ToolPolicyResolver, skill/MCP policy, and explicit `advisor_tool` approval. Blocking command/tool suggestions create explicit advisor evidence approvals even when PermissionEngine says the capability is `allow`; deny still blocks execution. Approved commands/tools execute through the normal command/tool pipeline with advisor audit metadata, and evidence status is satisfied only when the matching execution succeeds. Nothing is auto-run before approval. |
| Tool/MCP failure recovery | Earlier `failureKind` / `recoveryHint` was useful but still only a static hint. | Done: tool execution now asks `DecisionAdvisor` for `tool_recovery` action advice, records advisor/runtime-bounded proposals, publishes `agent.decision.tool_recovery`, and attaches `recoveryDecision` to the failed tool result and handoff. Runtime does not execute the recovery action automatically; safe follow-ups such as MCP refresh or advisor-selected fallback tools now become explicit approval-resume actions through the normal tool pipeline. |
...
| Provider recovery | Failure classification remains objective; no-partial failures are runtime-bounded, context-too-large gets only an objective compact retry, and advisor/fallback recovery requires partial output or preflight evidence. Provider preflight is now advisor-audited before risky calls; runtime may safely compact context, execute validated bounded task-splitting through the existing planning/DAG path, or switch this provider turn to an enabled switch-eligible fallback profile. | Done baseline: provider profile ranking uses persisted health metadata and records ranking/switch health into trace/proposal audit. Remaining: provider-specific health decay/latency scoring and richer UI history; auth/refusal stay non-retryable and user-visible. |

Still open / next implementation queue:

| Priority | Task | Current next step |
| --- | --- | --- |
| P0/P1 | Main workflow state machine phase 3 | User takeover now has a `user_takeover` DecisionAdvisor path with proposal audit; runtime still enforces task-state transitions. Stop/pause/continue remain bounded transitions, while wrap-up/change-target requests now write `mainWorkflow.convergence` with resumable handoff metadata and emit `task.user_takeover.convergence`. Cockpit now surfaces those convergence/handoff facts and can resume paused tasks through `task.resume`. Remaining work: richer budget dimensions and deeper automation policy controls. |
| P1 | Provider failure recovery phase 2 | Partial-aware recovery gating is implemented and tested: no-partial timeout does not call advisor or retry, context-too-large does one runtime compact retry, partial stream output can use advisor-gated non-stream fallback, auth stays non-retryable, recovery audit survives completed turns, and provider preflight can consult advisor before risky calls while auto-applying safe compaction, executing validated bounded task splitting through planning/DAG, or applying a guarded turn-scoped provider profile switch. Provider profile health/ranking now guards switch targets and is persisted in trace/proposal audit. Remaining work: provider-specific health history/latency scoring and deeper UI drill-down. |
| P1 | Product acceptance gate phase 2 | Static route/content health is covered for changed artifacts. `/api/...` references are now objective observations rather than hard gates. Changed README/docs artifacts now emit non-blocking `docs_quality_observation` advisories for objective signals such as placeholder copy, heading structure, fenced-code balance, and local relative links; changed artifacts also emit non-blocking `changed_surface_observation` advisories for objective path/content signals such as CLI entrypoints, persistence/migrations, tests, docs, UI/client artifacts, manifests, API routes/references, and data/config files. These feed product-surface/completion advisors instead of becoming fixed domain gates. LLM-guided product-surface classification runs before completion advisory, persists `product_surface_decision` proposals, and can request generic evidence such as design review, embedded hardware-in-loop proof, firmware build, migration dry-run, benchmark, API flow proof, docs consistency review, browser inspection, service/API proof, persistence round-trip, or MCP/tool evidence. Blocking advisor commands and non-command `suggestedTool` requests now create real approvals and can be resumed after approval; multi-step `suggestedCommands` / `suggestedTools` are expanded into independent evidence requests so evidence chains can be advisor-led without fixed runtime probes. Remaining work is wiring richer real evidence execution adapters when the relevant tools are available. |
| P1 | Frontend runtime cockpit phase 2 | Baseline drill-downs are implemented for acceptance audit, provider recovery/preflight signals, MCP/Skills signals, memory/context pressure, workspace status, and resumable handoff actions. Remaining work: richer automation controls, provider health/ranking history, and deeper run-report navigation. |
| P1 | Completion audit refinement | Completion advisor proposals persist to `proposal_records`, `completionEvidence.audit` carries reviewer/approval conclusions plus approval counts into advisor input/source/structured result/events, and cockpit drill-down exposes the audit trail. Remaining work: continue language/framework-specific verification matching. |
...
1. Product acceptance phase 2 follow-up: blocking advisor-suggested commands and non-command tool suggestions now turn into explicit advisor evidence approvals with advisor audit metadata and approval-resume execution, even when the underlying capability is configured as `allow`; policy `deny` still prevents execution. Multi-step `suggestedCommands` / `suggestedTools` are normalized into independent evidence requests, so advisor-led proof chains no longer need fixed runtime probes. README/docs quality and changed artifact surface observations now contribute advisory objective signals without hard-coding completion gates. Remaining work is wiring richer real evidence execution adapters when available, while keeping the evidence shape advisor-led rather than taxonomy-coded.
2. Provider recovery phase 2 baseline: provider preflight facts/advisor audit, safe compaction, executable bounded task splitting, guarded turn-scoped provider profile switching, and baseline provider health/ranking are done. Runtime still enforces retry budgets, partial-output gates, profile availability, permissions, approvals, DAG validation, and safety; next is provider-specific health history/latency scoring plus deeper UI drill-down.
3. Frontend runtime cockpit phase 2 plus main workflow phase 3: cockpit drill-down baseline is in place for acceptance/provider/MCP/context/workspace signals, and user takeover semantics plus resumable convergence are runtime-backed. Resumable handoff actions now have a first cockpit control path for paused tasks. Next focus is richer automation controls and budget convergence policies.
...
5. Language/framework verification matching and release-grade smoke coverage.
