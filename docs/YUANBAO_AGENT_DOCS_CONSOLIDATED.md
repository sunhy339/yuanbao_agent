...
| Product acceptance artifact gate | Done in current follow-up | Completion evidence now checks changed readable artifacts (`html/md/txt/css/js/jsx/ts/tsx/vue/svelte`) for visible mojibake/replacement-character tokens, validates local CSS/JS/image/route references from changed HTML, verifies changed HTML has visible route content, and runs `node --check` for reachable local scripts. Failed product-quality evidence routes write-oriented completion into `completion_review` instead of silently completing rough generated UI/docs. |
...
| P1 | Product acceptance gate phase 2 | Static route/content health is now covered for changed HTML. Next add true server/browser smoke, persistence/API flow, dynamic route/API health, and deeper README/docs quality beyond mojibake detection. |
...
1. Product acceptance phase 2: browser/server smoke, persistence/API flow, and dynamic route/API health.
