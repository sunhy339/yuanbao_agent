...
| P1 | Completion audit refinement | Completion advisor proposals persist to `proposal_records`, `completionEvidence.audit` carries reviewer/approval conclusions plus approval counts into advisor input/source/structured result/events, and cockpit drill-down exposes the audit trail. Language/framework-specific verification matching now supports advisor-accepted domain-specific sufficiency with auditable `verificationRequirements.advisorResolution`; remaining work is optional real-flow tuning and UI drill-down polish. |
