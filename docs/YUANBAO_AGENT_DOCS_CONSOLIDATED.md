...
| Advisor-requested follow-up evidence | Evidence execution must not be hard-coded to frontend/API/browser probes, and must not auto-run LLM-suggested commands or tools. | Third remediation done: `suggestedCommand` still becomes a permission-gated `run_command` suggestion, and `suggestedTool` becomes a generic tool suggestion with registry/capability/PermissionEngine metadata. Blocking command/tool suggestions now create explicit advisor evidence approvals even when PermissionEngine says the capability is `allow`; deny still blocks execution. Approved commands/tools execute through the normal command/tool pipeline with advisor audit metadata, and evidence status is satisfied only when the matching execution succeeds. Nothing is auto-run before approval. |
...
1. Product acceptance phase 2 follow-up: blocking advisor-suggested commands and non-command tool suggestions now turn into explicit advisor evidence approvals with advisor audit metadata and approval-resume execution, even when the underlying capability is configured as `allow`; policy `deny` still prevents execution. README/docs quality and changed artifact surface observations now contribute advisory objective signals without hard-coding completion gates. Remaining work is true server/browser/persistence flow verification when the task shape/advisor asks for that evidence, while keeping the evidence shape advisor-led rather than taxonomy-coded.
