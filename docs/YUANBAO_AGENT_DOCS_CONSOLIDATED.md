...
| Advisor-requested follow-up evidence | Evidence execution must not be hard-coded to frontend/API/browser probes, and must not auto-run LLM-suggested commands or tools. | Second remediation done: `suggestedCommand` still becomes a permission-gated `run_command` suggestion, and `suggestedTool` now becomes a generic tool suggestion with registry/capability/PermissionEngine metadata. Blocking tool suggestions can create `advisor_tool` approvals with advisor audit metadata; approved tools execute through the normal tool pipeline, and evidence status is satisfied only when the matching tool result succeeds. Nothing is auto-run before approval. |
