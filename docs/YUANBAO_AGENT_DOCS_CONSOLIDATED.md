...
| --- | --- | --- |
| P0/P1 | Main workflow state machine phase 3 | User takeover now has a `user_takeover` DecisionAdvisor path with proposal audit; runtime still enforces task-state transitions. Stop/pause/continue remain bounded transitions, while wrap-up/change-target requests now write `mainWorkflow.convergence` with resumable handoff metadata and emit `task.user_takeover.convergence`. Remaining work: richer budget dimensions and resumable handoff UI/cockpit drill-down recovery actions. |
...
| P1 | Frontend runtime cockpit phase 2 | Baseline drill-downs are implemented for acceptance audit, provider recovery/preflight signals, MCP/Skills signals, memory/context pressure, and workspace status. Remaining work: automation controls, resumable handoff actions, richer provider health/ranking visibility, and deeper run-report navigation. |
