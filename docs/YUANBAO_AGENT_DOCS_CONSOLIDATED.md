...
5. Generated-artifact / product acceptance — **Partial / advisor-led path active**:
   ...
   - Remaining: true server/browser/persistence flow verification when the task shape/advisor asks for that evidence, plus continued tuning of objective surface discovery.
...
| P1 | Product acceptance gate phase 2 | Static route/content health is covered for changed artifacts. `/api/...` references are now objective observations rather than hard gates. Changed README/docs artifacts now emit non-blocking `docs_quality_observation` advisories for objective signals such as placeholder copy, heading structure, fenced-code balance, and local relative links; changed artifacts also emit non-blocking `changed_surface_observation` advisories for objective path/content signals such as CLI entrypoints, persistence/migrations, tests, UI/client artifacts, manifests, API routes/references, and data/config files. These feed product-surface/completion advisors instead of becoming fixed domain gates. LLM-guided product-surface classification runs before completion advisory, persists `product_surface_decision` proposals, and can request generic evidence such as design review, embedded hardware-in-loop proof, firmware build, migration dry-run, benchmark, API flow proof, or docs consistency review. Blocking advisor commands and non-command `suggestedTool` requests now create real approvals and can be resumed after approval; multi-step `suggestedCommands` / `suggestedTools` are expanded into independent evidence requests so server/browser/persistence proof chains can be advisor-led without fixed runtime probes. Remaining work is wiring richer real browser/MCP/server execution flows when the relevant tools are available. |
...
1. Product acceptance phase 2 follow-up: blocking advisor-suggested commands and non-command tool suggestions now turn into explicit advisor evidence approvals with advisor audit metadata and approval-resume execution, even when the underlying capability is configured as `allow`; policy `deny` still prevents execution. Multi-step `suggestedCommands` / `suggestedTools` are normalized into independent evidence requests, so advisor-led server/browser/persistence proof chains no longer need fixed frontend/API/browser probes. README/docs quality and changed artifact surface observations now contribute advisory objective signals without hard-coding completion gates. Remaining work is wiring richer real browser/MCP/server execution flows when available, while keeping the evidence shape advisor-led rather than taxonomy-coded.
