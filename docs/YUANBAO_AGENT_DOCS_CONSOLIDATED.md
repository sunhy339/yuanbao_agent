...
| --- | --- | --- |
| P0/P1 | Main workflow state machine phase 3 | Done for the current backend main-flow baseline: user takeover has a `user_takeover` advisor path with proposal audit; stop/pause/continue remain bounded transitions; wrap-up/change-target requests write resumable `mainWorkflow.convergence`; automation controls and convergence policy are captured on task creation; step/context budget pressure is tracked in budget dimensions; pressure/exhaustion events are emitted; hard budget exhaustion invokes `budget_convergence` advisor and records resumable handoff metadata. Remaining work is optional deeper policy configuration and UI controls, not a blocker for the backend main-flow closure. |
