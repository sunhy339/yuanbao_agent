...
1. Product acceptance phase 2 follow-up: blocking advisor-suggested commands and non-command tool suggestions now turn into explicit advisor evidence approvals with advisor audit metadata and approval-resume execution, even when the underlying capability is configured as `allow`; policy `deny` still prevents execution. README/docs quality now contributes advisory objective signals without hard-coding completion gates. Remaining work is richer domain signal discovery while keeping the evidence shape advisor-led rather than taxonomy-coded.
