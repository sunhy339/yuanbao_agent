...
| Advisor-requested follow-up evidence | Evidence execution must not be hard-coded to frontend/API/browser probes, and must not auto-run LLM-suggested commands or tools. | Sixth remediation done: advisor evidence now has a domain-neutral executor state machine. Runtime emits `advisorEvidenceExecutor` records in completion evidence/audit and publishes `agent.evidence.executor.updated` transitions for requested, approval_requested, running, satisfied, failed, rejected, and policy-blocked states. `suggestedCommand` still becomes a permission-gated `run_command` suggestion, and `suggestedTool` becomes a generic tool suggestion with registry/capability/PermissionEngine metadata. Advisor evidence requests may carry multi-step `suggestedCommands` / `suggestedTools`; runtime normalizes them into independent evidence requests so each step gets its own permission/approval/execution state. Dynamic MCP suggested tools are no longer rejected merely because they lack a static capability mapping: registered `mcp__...` tools pass through ToolPolicyResolver, skill/MCP policy, and explicit `advisor_tool` approval. Blocking command/tool suggestions create explicit advisor evidence approvals even when PermissionEngine says the capability is `allow`; deny still blocks execution. Approved commands/tools execute through the normal command/tool pipeline with advisor audit metadata, and evidence status is satisfied only when the matching execution succeeds. Nothing is auto-run before approval. |
