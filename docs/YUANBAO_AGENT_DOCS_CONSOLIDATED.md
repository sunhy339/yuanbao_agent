...
1. MCP + Skills acceptance coverage — **Partial / mostly done**:
   - Done: skill-routed MCP tool call, `inherit_mcp` policy, compaction survival, failed MCP/tool handoff via `handoffSummary.failedTools`, structured failure kind/recovery hint for failed tools, and advisor-requested MCP evidence tools routed through ToolPolicyResolver plus explicit `advisor_tool` approval.
   - Remaining: missing skill fallback and turning unavailable server / partial response / permission-denied hints into advisor-selected recovery actions.
...
4. MCP + Skills fallback polish: advisor-requested MCP evidence tools now pass through ToolPolicyResolver and explicit approval; failed MCP/tool handoff includes structured `failureKind` and `recoveryHint`. Missing skill fallback and turning unavailable server / partial response / permission-denied hints into advisor-selected recovery actions remain.
