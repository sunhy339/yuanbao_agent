...
1. MCP + Skills acceptance coverage — **Partial / mostly done**:
   - Done: skill-routed MCP tool call, `inherit_mcp` policy, compaction survival, failed MCP/tool handoff via `handoffSummary.failedTools`, and advisor-requested MCP evidence tools routed through ToolPolicyResolver plus explicit `advisor_tool` approval.
   - Remaining: missing skill, unavailable server, partial response, and richer permission-denied fallback strategy selection.
...
4. MCP + Skills fallback polish: advisor-requested MCP evidence tools now pass through ToolPolicyResolver and explicit approval; missing skill/unavailable server/partial response strategy selection remains, while failed MCP/tool handoff is structured.
