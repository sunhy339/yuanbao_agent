...
| Claude Code/haha-cc alignment | `docs/yuanbao-claude-code-alignment-plan.md` / `docs/yuanbao-claude-code-alignment-todolist.md` | Yuanbao-facing alignment track complete for non-Computer-Use output parity: Yuanbao primary flat ServerMessage path, legacy `haha*` compatibility, status/tool/thinking/task/team/session events, AskUser/PlanMode/AgentTool, and frontend Yuanbao subscription tests are covered; Computer Use and external adapter ecosystem remain a later track |
