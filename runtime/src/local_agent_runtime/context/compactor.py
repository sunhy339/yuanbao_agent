...
class ContextCompactor:
    ...
    def _truncate_tool_outputs(
        ...
        Tool-role messages with a ``resultSummary`` field (generated by
        ``ToolUseSummaryGenerator``) prefer that summary over head/tail
        truncation.  Only messages without a summary are truncated using
        ...
        for msg in messages:
            ...
            if role == "tool" and isinstance(content, str) and len(content) > cls._TOOL_OUTPUT_MAX_CHARS:
                # Prefer pre-generated summary if available (from ToolUseSummaryGenerator)
                summary = msg.get("resultSummary")
                if summary and isinstance(summary, str) and summary.strip():
                    new_msg = dict(msg)
                    new_msg["content"] = f"[Summary] {summary.strip()}"
