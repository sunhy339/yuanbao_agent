...
register_decision(DecisionKindEntry(
    ...
    description=(
        ...
        "runtime still enforces budgets, permissions, and safety gates. Do not add "
        "workspace evidence gates here; the model should request read-only tools when "
        "workspace grounding is needed."
    ),
    required_input_fields=("goal",),
...
register_decision(DecisionKindEntry(
    ...
    description=(
        ...
        "action such as proceeding, compacting context, asking the user, proposing a "
        "split, or switching to a validated provider profile. The runtime may auto-apply "
        "safe context compaction, execute a validated bounded split through the existing "
        "planning/DAG path, or apply a turn-scoped provider switch when fallbackProviderId "
        "resolves to an available profile; user-intervention advice remains audited."
    ),
    ...
    def _normalize_payload(kind: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if kind == "routing_strategy":
            ...
            if isinstance(continuation, dict) and isinstance(camel_continuation, dict):
                ...
                normalized.pop("toolContinuation", None)
            normalized.pop("workspace_evidence_required", None)
