...
class TestDAGExecutorExecute:
    ...
    def test_failed_dispatch_status_can_use_second_continuation_when_progress_exists(self) -> None:
        class MultiRetrySubagent:
            def __init__(self) -> None:
                self.calls: list[dict[str, Any]] = []
                self._count = 0
            ...
            def dispatch(self, params: dict[str, Any]) -> dict[str, Any]:
                ...
                if self._count < 3:
                    return {
                        "status": "failed",
                        "summary": "Still fixing pytest failures.",
                        "error": {
                            "message": "Still fixing pytest failures.",
                            "partialHandoff": {
                ...
                return {"status": "completed", "summary": "continued twice and tests pass"}

        subtasks = [
            Subtask(id="a", title="Tests", description="Write incident tests", dependencies=[]),
        ]
        plan = _make_plan(subtasks)
        subagent = MultiRetrySubagent()
        executor = DAGExecutor(subagent)
